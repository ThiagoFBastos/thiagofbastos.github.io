...
$f(0, 0) = 1 \pmod{10^{9} + 7}$ <br>
$f(i, j) = ((j - 1) * f(i, j - 1) + f(i - 1, j - 1)) \pmod{10^{9} + 7}$
...
$dp(i) = (dp(i) + f(i, j) * \binom{n}{j} * n^{n - j}) \pmod{10^{9} + 7}$ $(\forall\,1 \le j \le n)$
...
$dp(j) = (dp(j) - dp(i) * \binom{i}{j}) \pmod{10^{9} + 7}$
