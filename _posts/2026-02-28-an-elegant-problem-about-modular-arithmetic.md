...
The main idea behind the solution to this problem is to count, for each $1 \le i \le \left\lfloor \frac{pn}{q} \right\rfloor$, which values $pj$, with $1 \le j \le n$, satisfy $pj \ge qi$, and then subtract q from $p\frac{n(n + 1)}{2}$, which can be done recursively.
